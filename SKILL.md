...
description: |
  ...
  voice, negative parallelisms, and filler phrases. Includes French-specific
  vocabulary and register patterns.
license: MIT
...
### Worked example: default voice vs. matched voice (French)

A rewrite that clears every AI pattern can still sound like *a* human instead of *this* human. Compare the same passage humanized with the default voice against a rewrite matched to a specific writing sample.

**Sample provided by the writer (used only to extract voice patterns, not copied):**
...
Voice patterns in the sample: mid-length sentences with one deliberate short fragment for emphasis; register casual enough to use "truc" and "boîtes" but not slangy; opens a sentence with "Et" as a rhythm break; prefers subordination ("Là où X, Y", "puisque") over punctuation connectors; drops "c'est que" entirely from "ce qui est sûr".
...
Both are clean of AI patterns. Only the matched version sounds like the person who wrote the sample: the semicolon becomes a "Là où X, Y" subordination, the colon becomes "puisque," "c'est que" gets dropped, and the standalone "Et ce..." fragment is kept rather than folded back in.
...
**Reach for the one exact word.** A specific, slightly unusual word used correctly reads as human; a safe, generic one reads as AI. Don't say "problem" when you mean "the timing belt is shredding itself."

**Talk to the reader.** Ask a real question. Use "you" when appropriate. Human writers build their argument with the reader; AI prose asserts conclusions at the reader.
...
- **Long, complex sentences.** In academic and legal writing especially, long sentences with multiple subordinate clauses are deliberate and correct. Don't shorten them just because they read slowly.
...
Apply these in addition to, not instead of, the sections above when humanizing French text.

### F1. Overused AI vocabulary in French

**Mots à surveiller :** au cœur de, un enjeu majeur/de taille, véritable, incontournable, façonner, s'inscrire dans une démarche de, à l'ère de, dans un monde en constante évolution, de plus en plus, il convient de noter que, en effet (utilisé comme remplissage), par ailleurs, de surcroît, riche en, une pluralité de, indéniable, jouer un rôle clé/crucial, témoigner de, un tournant décisif, transformer en profondeur, un écosystème.

**Problème :** Ces mots se retrouvent en grappe dans les textes IA en français exactement comme "delve", "tapestry" ou "testament" en anglais. Pris isolément ils sont ordinaires ; plusieurs ensemble, surtout "au cœur de" ou "un enjeu majeur" associés à "véritable", forment le signal.
...
### F2. Calques anglicisés et glissement de registre

**Problème :** Le français IA emprunte parfois le sens anglais d'un mot plutôt que le sens français correct ("réaliser" pour "se rendre compte", "supporter" pour "soutenir", "opportunité" pour "occasion"). Il tend aussi vers un registre rigide et sur-formel (nominalisations lourdes, vouvoiement systématique) même quand le contexte appellerait un ton plus direct.
...
### F3. Introductions par métaphore spatiale

**Problème :** Le français IA a adopté le calque de l'ouverture anglaise "in the ever-evolving landscape of X" : "Dans le paysage [actuel/moderne/contemporain] de..." utilisé pour ouvrir une section. C'est la version française du même signal déjà identifié en §1. Le correctif est le même : nommer le sujet réel plutôt que de le cadrer dans une métaphore de décor.
...
### F4. Aplatissement du registre régional

**Problème :** Le français IA bascule vers un registre standard hexagonal neutre, même quand le texte source porte une voix régionale — français ivoirien, québécois, belge, suisse. Il efface le lexique, la syntaxe et les tournures régionales au profit d'un "français neutre". Ne pas corriger ces régionalismes : les préserver est un signal humain fort, de la même façon qu'un éditeur anglophone maintient les graphies britanniques ou américaines plutôt que de les convertir.

**Exemples à préserver (français ivoirien) :** *wôrô-wôrô* (taxi collectif), *gbaka* (minibus), *tantie/tonton* en apostrophe directe, *dja* (c'est bon / d'accord), constructions comme "il a gâté la voiture" (il a abîmé la voiture).

**Principe :** Si la voix régionale est présente dans le texte source, la renforcer plutôt que l'effacer. Si le texte source est censé avoir une voix régionale mais en est dépourvu, ne pas en inventer une — c'est au-delà du rôle de l'éditeur.
...
Clean reset from v2.13.0. Restored blader v2.8.2 as the base (622 lines, 33 numbered patterns). Kept only six additions from the v2.9.0–v2.13.0 runs that are actionable in practice:

- Added "Reach for the one exact word" and "Talk to the reader" to PERSONALITY AND SOUL
- Added Voice Calibration worked example in French (from v2.12.1, with consent)
- Added false-positive bullet about long complex sentences in academic/legal writing
- Added FRENCH-SPECIFIC PATTERNS: F1 (AI vocabulary), F2 (calques/register), F3 (spatial-metaphor intro), F4 (regional register for African French)
- Added EXTENSION PATTERNS placeholder with a strict bar for future additions

Dropped from v2.9.0–v2.13.0: burstiness score (0.65–0.85), F1 typography/punctuation injection rule, §34–48 academic patterns, German/Italian/Portuguese/Spanish sections, research apparatus. These were either causing contradictions (em dashes: §14 removes them, but PERSONALITY "parenthetical aside" added them back), too theoretical to apply consistently, or irrelevant to current use cases.
