...
- **Patterns:** the skill currently defines **45 numbered patterns**, plus French, Spanish, German, Italian, and Portuguese-specific sections. If you add, remove, or renumber any, update the README pattern table, its "N Patterns Detected" heading, and every cross-reference in the same change. Keep numbering stable unless you are deliberately renumbering. (Note: the README table has drifted out of sync across several recent runs - see the note at the top of its patterns section. Fixing that fully is follow-up work, not a blocker for adding new patterns to `SKILL.md` itself.)
